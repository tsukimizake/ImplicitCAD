...
import Prelude (Applicative, Bool (False, True), Either (Left, Right), Num, abs, atan2, ceiling, cos, either, floor, fromInteger, id, max, min, minimum, otherwise, pi, pure, round, sin, sqrt, ($), (*), (**), (+), (-), (.), (/), (/=), (==), (||))
...
getImplicit3 _ (Cylinder h r1 r2) = \(V3 x y z) ->
  let d = sqrt (x * x + y * y) - ((r2 - r1) / h * z + r1)
      θ = atan2 (r2 - r1) h
   in max (d * cos θ) (abs (z - h / 2) - (h / 2))
...
getImplicit3 ctx (Extrude h symbObj) =
  let obj = getImplicit symbObj
   in \(V3 x y z) -> rmax (objectRounding ctx) (obj (V2 x y)) (abs (z - h / 2) - h / 2)
getImplicit3 ctx (ExtrudeM twist scale translate symbObj height) =
  let r = objectRounding ctx
      ...
      scaleVec :: ℝ -> ℝ2 -> ℝ2
      scaleVec z (V2 x y) =
        let (V2 sx sy) = toScaleFn scale z
         in V2 (x / sx) (y / sy)
      rotateVec :: ℝ -> ℝ2 -> ℝ2
      rotateVec θ (V2 x y)
        | θ == 0 = V2 x y
...
getImplicit3 _ (ExtrudeOnEdgeOf symbObj1 symbObj2) =
  let obj1 = getImplicit symbObj1
      obj2 = getImplicit symbObj2
   in \(V3 x y z) -> obj1 $ V2 (obj2 (V2 x y)) z
        ...
        let r = sqrt $ x * x + y * y
            ...
            ns =
              if capped
                ...
                else [0 .. floor $ (totalRotation - θ) / tau]
        n <- ns
        let θvirt = fromℕtoℝ n * tau + θ
            (V2 rshift zshift) = translate' θvirt
            twist = rotate' θvirt
            rz_pos =
              if twists
                then
                  let (c, s) = (cos twist, sin twist)
                      (r', z') = (r - rshift, z - zshift)
                   in V2 (c * r' - s * z') (c * z' + s * r')
