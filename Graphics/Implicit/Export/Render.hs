...
import Control.Lens (over, view, _Just, _Wrapped)
import Control.Parallel.Strategies (parBuffer, rdeepseq, using)
import Data.Foldable (fold)
import Data.Maybe (fromMaybe)
import Graphics.Implicit.Definitions (Fastℕ, Obj2, Obj3, Polyline (getSegments), SymbolicObj2, SymbolicObj3, TriangleMesh, fromℕ, fromℕtoℝ, ℕ, ℝ, ℝ2, ℝ3, ℝ3' (ℝ3'), (⋯/))
...
import Prelude (Int, ceiling, div, error, fmap, foldMap, max, min, reverse, tail, traverse, ($), (*), (+), (-), (.), (<$>), (<>))
...
getMesh res@(V3 xres yres zres) symObj =
  let -- Grow bounds a little to avoid sampling at exact bounds
